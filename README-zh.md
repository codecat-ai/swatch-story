...
swatch-story 仍处于 alpha 阶段，但已经可用于本地、确定性的调色板提取和审阅报告。经过 2026-05-18 发布就绪复查，项目正从增长节奏转为维护节奏：每 2-4 周一次，并在出现紧急 bug、文档真实性、依赖或报告 schema 稳定性问题时处理。

现在：
- 保持即将到来的发布候选版本稳定：打标签前运行发布检查清单，关注用户报告的调色板/报告回归，并避免新增宽泛功能，除非它们能提升发布信心。

下一步：
- 仅在 gallery 示例能直接展示发布关键流程（如透明度、忽略背景或感知聚类）时改进它们。
- 在带标签发布记录当前 JSON 形状之前，保持报告 schema 变更为向后兼容的新增项。
- 在下一个发布标签之后，或真实用户反馈揭示更高价值的工作流缺口时，再复查节奏。
...
发布检查清单：
- 在干净的本地虚拟环境中运行 `ruff check .`、`ruff format --check .`、`pytest -q` 和 `python -m build`。
- 确认 `README.md`、`README-zh.md` 和 `README-ja.md` 描述相同的安装路径、示例、路线图状态和 MIT 许可证说明。
- 确认 `CHANGELOG.md` 为每个面向用户的 CLI 选项、报告格式字段或输出产物变更包含 Unreleased 条目。
- 当发布改变提取或报告渲染时，用小型本地夹具冒烟测试代表性的单图、对比、基准、批量和 gallery 命令。
- 除非包已明确发布并验证，否则不要添加包注册表安装命令。

完成复查：
- 已完成：2026-05-18 发布就绪复查将 swatch-story 降为维护节奏，因为当前面向用户的工作流已经足以支持发布候选版本，剩余条目主要是发布卫生或示例证据，而不是核心功能增长。
