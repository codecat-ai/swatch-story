...
Now:
- Lab クラスタリングのフィクスチャ/ドキュメント単位の後に、もう 1 回リリース準備レビューを行い、swatch-story を成長モードに残すか保守モードへ移すかを判断します。

Next:
- 透明度、無視する背景、知覚的クラスタリングの例を含むようにギャラリーサンプルを改善します。
- 検証コマンド、README 翻訳同期、changelog レビューをタグ準備に結び付ける短いリリースチェックリストを追加します。
- 完了レビューでユーザー向けの不足が見つからなければ、ロードマップを機能成長から保守へ移します。対象は bug 修正、依存関係の衛生、ドキュメント正確性、レポート schema 安定性です。
